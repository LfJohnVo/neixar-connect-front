...
@Component({
  selector: 'app-completed-vacancies',
  templateUrl: './completed-vacancies.component.html',
  styles: []
})
export class CompletedVacanciesComponent implements OnInit {
  Toast = Swal.mixin({
    toast: true,
    position: 'top-end',
  ...
  ngOnInit() {
    this.getCompletedRecruitments(1);
  }

  getCompletedRecruitments( page ) {
    this.spinner.show();    
    this._recruitmentService.getCompletedRecruitments( 'ALL', page).subscribe(
      res => {                
