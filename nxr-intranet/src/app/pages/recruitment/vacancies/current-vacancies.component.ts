...
@Component({
  selector: 'app-current-vacancies',
...
export class CurrentVacanciesComponent implements OnInit {
  ...
  ngOnInit() {
    this.getCurrentRecruitments(1);
  }

  getCurrentRecruitments( page ) {
    this.spinner.show();    
    this._recruitmentService.getCurrentRecruitments( 'ALL', page).subscribe(
      res => {        
